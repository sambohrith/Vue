...
const getChatList = async (currentUserId) => {
  ...
  const contacts = await Promise.all(users.map(async (user) => {
    ...
    try {
      if (lastMessage && lastMessage.createdAt && lastMessage.createdAt instanceof Date && !isNaN(lastMessage.createdAt.getTime())) {
        lastMessageTime = lastMessage.createdAt.toISOString().replace('T', ' ').substring(0, 19);
      }
    } catch (e) {
      console.error('Error formatting lastMessageTime:', e);
    }
    ...
    for (const message of messages) {
      ...
      if (!conversationMap.has(conversationKey)) {
        ...
        conversationMap.set(conversationKey, {
          ...
          lastMessageTime: message.createdAt && message.createdAt instanceof Date && !isNaN(message.createdAt.getTime()) ? new Date(message.createdAt).toISOString().replace('T', ' ').substring(0, 19) : ''
        });
      }
