...
const getPosts = async (currentUserId, page = 1, limit = 10) => {
  ...
  const posts = await Promise.all(rows.map(async (post) => {
    ...
    return {
      ...
      isPublic: true,
      createdAt: post.createdAt.toISOString().replace('T', ' ').substring(0, 19),
      updatedAt: post.updatedAt.toISOString().replace('T', ' ').substring(0, 19),
      likes: likeCount,
...
const addComment = async (postId, userId, req) => {
  ...
  return {
    ...
    createdAt: comment.createdAt.toISOString().replace('T', ' ').substring(0, 19)
  };
};
